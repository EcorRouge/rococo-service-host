...
class TestConfigFactory(unittest.TestCase):
    ...
    def test_setup_rabbitmq_params_invalid_threads(self):
        ...
        self.config.get_env_var = MagicMock(side_effect=self._mock_env_var(env_vars))
        # int('invalid') raises ValueError, not TypeError, so this should still succeed
        # but the exception handling is for TypeError which won't be caught
        try:
            result = self.config._setup_rabbitmq_params()
            # If ValueError is raised and not caught, test should handle it
        except ValueError:
            result = False
        self.assertFalse(result)
